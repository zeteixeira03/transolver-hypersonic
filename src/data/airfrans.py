...
def compute_norm_stats(arrays: list[np.ndarray]) -> NormStats:
    ...
    eps = 1e-8
    return NormStats(
        x_mean=torch.tensor(x.mean(axis=0), dtype=torch.float32),
        x_std=torch.tensor(x.std(axis=0) + eps, dtype=torch.float32),
        y_mean=torch.tensor(y.mean(axis=0), dtype=torch.float32),
        y_std=torch.tensor(y.std(axis=0) + eps, dtype=torch.float32),
    )
