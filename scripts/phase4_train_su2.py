...
def split_paths(
    ...
    splits = {
        "train": [p for gid in sorted(train_geoms) for p in core_by_geom[gid]],
...
def evaluate(
    ...
    *,
    ...
    if per_case_qw:
        ...
        out["n_qw_cases"] = int(len(per_case_qw))
    return out
...
def main() -> None:
    ...
    splits = split_paths(paths, groups, geom_ids, args.val_frac, args.test_frac, args.seed)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.val_every == 0 or epoch == args.epochs:
            val = evaluate(model, val_ds, stats, args.device)
    ...
    # final eval on test + OOD slabs from the best checkpoint
    model.load_state_dict(torch.load(out / "best.pt", map_location=args.device))
    final = {}
    for split_name in ["val", "test", *(f"ood_{g}" for g in OOD_GROUPS)]:
        if not splits[split_name]:
            continue
        ds = SU2Dataset(splits[split_name], stats, subsample=None)
        final[split_name] = evaluate(model, ds, stats, args.device)
